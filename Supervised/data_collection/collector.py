...
data_folder = 'Supervised/data/using mss2'
...
with mss.mss() as sct:
    while run:
        win_info = get_window_info(title)
        if not win_info:
            print(f"Window '{title}' not found. Pausing capture.")
            time.sleep(1)
            continue
        win_pos, res = win_info
        monitor = {"top": win_pos[1], "left": win_pos[0], "width": res[0], "height": res[1]}
        img = sct.grab(monitor)

        # Process the screenshot
        frame = np.array(img)
        frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
        frame = frame[100:-50, 5:-5]
        frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)

        # Calculate control inputs
        steering = 1  # Default to neutral (1)
        throttle = 0  # Default to no throttle
